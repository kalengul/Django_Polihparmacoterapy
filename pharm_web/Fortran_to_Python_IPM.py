...
def load_data_in_file(BASE_DIR,file_name,nj):
    ...
    for k in range(1, n_k):
        ...
        side_effects.append({'name': disease.name, 'class': int(nom[k]), 'rangsum': round(float(rangsum[k]),2)})

        # Группировка элементов по номерам
        #groups[nom].append(context[k])
    # Обновление context с элементами из arr
    sorted_side_effects = sorted(side_effects, key=lambda x: x['rangsum'], reverse=True)
    # Фильтрация по классам
    class_1 = [effect for effect in sorted_side_effects if effect['class'] == 1]
    class_2 = [effect for effect in sorted_side_effects if effect['class'] == 2]
    class_3 = [effect for effect in sorted_side_effects if effect['class'] == 3]
    context.update({'side_effects_class_1':class_1, 'side_effects_class_2':class_2, 'side_effects_class_3':class_3})
    #print('context:')
    #print(context)
    ...
    for k in range(0, len(drugs_class_1)):
        ...
        # Сохранение названия и значения в контексте
        arr_drugs.append({'name': drugs.name[0].upper() + drugs.name[1:], 'class': 1})
    ...
    for k in range(1, len(drugs_class_2)):
        ...
        # Сохранение названия и значения в контексте
        arr_drugs.append({'name': drugs.name[0].upper() + drugs.name[1:], 'class': 2})
    ...
    for k in range(1, len(drugs_class_3)):
        ...
        # Сохранение названия и значения в контексте
        arr_drugs.append({'name': drugs.name[0].upper() + drugs.name[1:], 'class': 3})
...
def go_all_iteractions(request,BASE_DIR):
    ...
    drugs = drugs_chf.objects.filter(name__in=[name.lower() for name in drug_names])
